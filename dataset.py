...
from .dataset import VideoFolderInstance, BatchImageFolderInstance, MOS_dataloader, VideoFolderInstance2, VideoFolderInstance2_MOS
...
def build_transforms(aug, modal, use_memory_bank=True):
    ...
    if aug == 'A':
        ...
        # used in MoCoV2
        train_transform = transforms.Compose([
            # transforms.RandomResizedCrop(400, scale=(crop, 0.5)),
            # transforms.RandomCrop((500,300),),# scale=(crop, 1.)),
            # transforms.RandomHorizontalFlip(),
        ...
        # used in MoCoV2
        train_transform = transforms.Compose([
            # transforms.RandomCrop(300,),# scale=(crop, 1.)),
...
def build_contrast_loader(opt, ngpus_per_node):
    ...
    crop_size=512
    if use_jigsaw:
        train_dataset = BatchImageFolderInstance(
            train_dir, transform=train_transform, crop_size=crop_size,
            two_crop=(not use_memory_bank), 
            jigsaw_transform=jigsaw_transform
        )
    else:
        train_dataset = VideoFolderInstance2(root=train_dir, 
            transform=train_transform, crop_size=crop_size, crop_type='random', 
            two_crop=True, target_transform=None
        )
...
def build_MOS_loader(opt, ngpus_per_node):
    """build loaders for MOS training"""
    data_folder = opt.data_folder_MOS
    batch_size = int(opt.batch_size / opt.world_size)
    num_workers = int((opt.num_workers + ngpus_per_node - 1) / ngpus_per_node)
    ...
    train_dataset = VideoFolderInstance2(root=train_dir, 
            transform=train_transform, crop_size=crop_size, crop_type='random', 
            two_crop=False, target_transform=target_normalize())
    ...
    test_dataset = loader = VideoFolderInstance2(root=test_dir, 
        transform=train_transform, crop_size=crop_size, crop_type='random', 
        two_crop=False, target_transform=target_normalize())
...
def build_contrast_loader_testing(opt, data_folder):
    """build loaders for contrastive training"""
    ...
    train_transform, jigsaw_transform = \
        build_transforms('testing_B', modal, use_memory_bank)

    # train_dir = os.path.join(data_folder, 'train')
    if use_jigsaw:
        train_dataset = VideoFolderInstance(
            data_folder, transform=train_transform,
            two_crop=False,
            jigsaw_transform=jigsaw_transform
        )
    else:
